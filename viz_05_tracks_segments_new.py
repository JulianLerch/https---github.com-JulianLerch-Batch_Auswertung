...
# -*- coding: utf-8 -*-
"""
Visualization Module 05 - New Segmented Tracks
Ordner: 05_Tracks_New_Segments

Trajektorien mit neuen Segmenten (ohne DIRECTED):
- DIRECTED → SUPERDIFFUSION/NORMAL/SUBDIFFUSION
- Farbcodiert nach neuen Klassen
- Scalebar
- Legende
"""
...
# =====================================================
#          SEGMENTED TRACK PLOT (NEW)
# =====================================================

def plot_segmented_trajectory_new(trajectories, segment_annotations, traj_id, 
                                  save_path=None, scalebar_length=None):
    """
    Plottet Trajektorie mit neuen Segmenten (ohne DIRECTED).
    
    Args:
        trajectories: dict {traj_id: trajectory}
        segment_annotations: dict {traj_id: [segments]} 
                           (Segmente sollten 'final_class' haben)
    ...
    if traj_id in segment_annotations:
        ...
        for segment in segment_annotations[traj_id]:
            ...
            if segment_coords and len(segment_coords) >= 2:
                ...
                class_name = segment.get('final_class', segment.get('class'))
                
                # Farbe aus neuen Farben (mit SUPERDIFFUSION!)
                color = NEW_COLORS.get(class_name, 'black')
    ...
    if PLOT_SHOW_TITLE:
        ax.set_title(f'Trajektorie {traj_id} - Reklassifizierte Segmente',
...
def create_all_segmented_tracks_new(trajectories, segment_annotations_new, 
                                    output_folder, scalebar_length=None):
    """
    Erstellt Segment-Plots (Neu) für alle Trajektorien.
    
    Args:
        trajectories: dict {traj_id: trajectory}
        segment_annotations_new: dict {traj_id: [segments]} mit finalen Klassen
    ...
    logger.info(f"Erstelle {n_total} Segmented Track Plots (Neu)...")
    
    for idx, traj_id in enumerate(trajectories.keys()):
        save_path = os.path.join(output_folder, f'track_{traj_id:04d}_segments_new.svg')
        plot_segmented_trajectory_new(trajectories, segment_annotations_new, traj_id, 
                                     save_path, scalebar_length)
        
        if (idx + 1) % PROGRESS_INTERVAL_TRACKS == 0:
            logger.info(f"  Verarbeitet: {idx + 1}/{n_total}")
    
    logger.info(f"✓ {n_total} Segmented Track Plots (Neu) erstellt")
